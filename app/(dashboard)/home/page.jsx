...
import { useAllSupplierLedgers, useAllBuyerLedgers } from "@/lib/hooks/useLedger"
...
const getImageArray = (item) => {
  ...
  if (item.productType?.images && Array.isArray(item.productType.images) && item.productType.images.length > 0) return item.productType.images;
  return [];
};
...
function StatCard({ label, value, icon: Icon, trend, trendValue, loading, color = "primary", onClick, description }) {
  if (loading) {
    return (
  ...
  const themes = {
    primary: { icon: "text-blue-600 bg-blue-50/50", border: "border-blue-100/50", glow: "group-hover:shadow-blue-500/10" },
    success: { icon: "text-emerald-600 bg-emerald-50/50", border: "border-emerald-100/50", glow: "group-hover:shadow-emerald-500/10" },
    warning: { icon: "text-amber-600 bg-amber-50/50", border: "border-amber-100/50", glow: "group-hover:shadow-amber-500/10" },
    danger: { icon: "text-rose-600 bg-rose-50/50", border: "border-rose-100/50", glow: "group-hover:shadow-rose-500/10" },
    purple: { icon: "text-purple-600 bg-purple-50/50", border: "border-purple-100/50", glow: "group-hover:shadow-purple-500/10" },
  }
  ...
  return (
    ...
    >
      ...
      <div className={cn("absolute -right-4 -top-4 w-24 h-24 rounded-full opacity-[0.03] transition-transform duration-500 group-hover:scale-150", theme.icon)}></div>
      ...
      <CardContent className="p-6 relative z-10">
        <div className="flex items-center justify-between space-y-0 pb-2">
          ...
          <div className={cn("p-2.5 rounded-xl transition-all duration-300 group-hover:rotate-12", theme.icon)}>
            <Icon className="h-5 w-5" />
          </div>
        </div>
        
        <div className="flex items-baseline justify-between mt-3">
          <h3 className="text-3xl font-black tracking-tight text-slate-900 leading-none">{value}</h3>
        </div>
        ...
        {trendValue ? (
          <div className="flex items-center mt-5">
            ...
            <span className="text-[10px] text-slate-400 ml-2 font-medium">this week</span>
          </div>
        ) : (
          <div className="flex items-center mt-5 text-[10px] text-slate-400 font-medium group-hover:text-slate-600 transition-colors">
            View details <ArrowRight className="h-3 w-3 ml-1 group-hover:translate-x-1 transition-transform" />
          </div>
        )}
      </CardContent>
    </Card>
...
function QuickAction({ label, icon: Icon, onClick, color = "blue", description }) {
  const themes = {
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        {description && <span className="text-[11px] text-white/70 block mt-0.5 leading-tight">{description}</span>}
      </div>
    </button>
  )
}
...
export default function HomePage() {
  ...
  const { data: suppliersData, isLoading: suppliersLoading } = useSuppliersReport()
  ...
  const { data: supplierLedgerData, isLoading: supplierLedgerLoading } = useAllSupplierLedgers({ limit: 10 })
  const { data: buyerLedgerData, isLoading: buyerLedgerLoading } = useAllBuyerLedgers({ limit: 10 })
            ...
            <CardHeader className="flex flex-row items-center justify-between p-8 pb-4">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-xl font-black text-slate-900 tracking-tight">Sales Intelligence</CardTitle>
                </div>
                <CardDescription className="text-slate-400 font-medium text-xs">Revenue trend analysis for the current week</CardDescription>
              </div>
              <div className="flex items-center gap-2 bg-slate-50 p-1 rounded-xl">
                <Badge variant="ghost" className="bg-white text-blue-600 shadow-sm text-[10px] font-bold border-none">WEEKLY</Badge>
                <Badge variant="ghost" className="text-slate-400 text-[10px] font-bold border-none">MONTHLY</Badge>
              </div>
            </CardHeader>
            ...
            <CardHeader className="p-8 pb-4">
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <CardDescription className="text-slate-400 font-medium text-xs">Real-time financial movement across all accounts</CardDescription>
                </div>
                <Button 
                  variant="outline" 
                  size="sm" 
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {[
                      ...(supplierLedgerData?.entries || []).map(entry => ({
                        ...entry,
                        partyType: 'Supplier',
                        partyName: entry.supplier?.name || entry.supplier?.company || 'Unknown'
                      })),
                      ...(buyerLedgerData?.entries || []).map(entry => ({
                        ...entry,
                        partyType: 'Customer',
                        partyName: entry.buyer?.name || entry.buyer?.company || 'Unknown'
                      }))
                    ]
                        ...
                        <tr key={idx} className="hover:bg-slate-50/30 transition-colors group">
                          <td className="px-8 py-5">
                            <div className="flex flex-col">
                              <span className="font-bold text-slate-900 group-hover:text-blue-600 transition-colors">{entry.partyName}</span>
                              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter mt-0.5">
                                {entry.date ? new Date(entry.date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' }) : "—"} • {entry.partyType}
                              </span>
                            </div>
                          </td>
                          <td className="px-8 py-5">
                            <Badge variant="outline" className="rounded-lg text-[9px] font-black uppercase tracking-widest border-slate-100 bg-white shadow-sm px-2">
                              {entry.type || 'General'}
                            </Badge>
                          </td>
        ...
        <div className="xl:col-span-4 space-y-8">
            ...
            <CardHeader className="p-6 pb-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <CardTitle className="text-lg font-black text-slate-900 tracking-tight">Stock Alerts</CardTitle>
                </div>
                {lowStockCount > 0 && <Badge className="bg-rose-500 hover:bg-rose-600 shadow-lg shadow-rose-200 border-none text-[10px] font-black px-2">{lowStockCount}</Badge>}
              </div>
            </CardHeader>
            <CardContent className="p-0">
              <div className="divide-y divide-slate-50">
                {inventoryItems.filter(item => item.lowStock || item.needsReorder).slice(0, 4).map((item, idx) => (
                  <div 
                    key={idx} 
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-slate-900 truncate tracking-tight">{item.productName}</p>
                      <p className="text-[10px] text-rose-500 font-black uppercase tracking-widest mt-0.5">Stock level: {item.currentStock}</p>
                    </div>
                ...
                {lowStockCount === 0 && (
                  <div className="p-10 text-center">
                    ...
                    <p className="text-sm font-bold text-slate-900 uppercase tracking-widest">Inventory Secure</p>
                    <p className="text-xs text-slate-400 font-medium mt-1">All items are within healthy limits</p>
                  </div>
                )}
              </div>
              {lowStockCount > 4 && (
          ...
          <Card className="border-none shadow-[0_8px_30px_rgb(0,0,0,0.04)] bg-gradient-to-br from-indigo-600 to-blue-700 rounded-3xl text-white">
            <CardHeader className="pb-2">
              <CardTitle className="text-lg font-black tracking-tight">Monthly Target</CardTitle>
              <CardDescription className="text-white/60 text-xs font-medium">Progress towards sales objective</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4 pt-4">
                ...
                <p className="text-[10px] font-bold text-white/50 uppercase tracking-widest text-center">£12,450 remaining to reach goal</p>
              </div>
            </CardContent>
          </Card>
        </div>
  ...
  const tabs = [
      ...
      content: (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <StatCard label="Stock Valuation" value={currency(totalStockValue)} icon={Package} loading={inventoryLoading} color="purple" />
            <StatCard label="Total SKU Count" value={inventoryItems.length.toString()} icon={Target} loading={inventoryLoading} color="primary" />
            <StatCard label="Low Stock Warning" value={lowStockCount.toString()} icon={AlertTriangle} loading={inventoryLoading} color="danger" />
            <StatCard label="Net Items" value={inventoryItems.reduce((s, i) => s + (i.currentStock || 0), 0).toLocaleString()} icon={Monitor} loading={inventoryLoading} color="success" />
          </div>

          <Card className="border-none shadow-sm bg-white rounded-3xl overflow-hidden">
            <CardHeader className="p-8">
              <CardTitle className="text-xl font-black text-slate-900 tracking-tight">Warehouse Ledger</CardTitle>
              <CardDescription>Live inventory tracking and stock valuation</CardDescription>
            </CardHeader>
            <CardContent className="p-0">
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  ...
                  <tbody>
                    {inventoryItems.slice(0, 15).map((item, idx) => (
                      <tr key={idx} className="hover:bg-slate-50/30 transition-colors border-b border-slate-50 last:border-0">
                        <td className="px-8 py-4">
                          <div className="flex items-center gap-4">
                            <div className="h-10 w-10 rounded-xl overflow-hidden border border-slate-100 shadow-sm bg-white">
                              <ProductImageGallery images={getImageArray(item)} alt={item.productName} size="sm" maxVisible={1} />
                            </div>
                            <span className="font-bold text-slate-900">{item.productName}</span>
                          </div>
                        </td>
                        <td className="px-8 py-4 font-bold text-slate-400 font-mono text-[11px]">{item.sku}</td>
                        <td className="px-8 py-4 text-right font-black text-slate-900 tabular-nums">{item.currentStock}</td>
                        <td className="px-8 py-4 text-right font-black text-slate-900 tabular-nums">{currency(item.totalValue || 0)}</td>
                        <td className="px-8 py-4 text-center">
                          {item.lowStock || item.needsReorder ? (
                            <Badge className="bg-rose-50 text-rose-600 border-rose-100 text-[9px] font-black uppercase tracking-widest px-2">Low Stock</Badge>
                          ) : (
                            <Badge className="bg-emerald-50 text-emerald-600 border-emerald-100 text-[9px] font-black uppercase tracking-widest px-2">Healthy</Badge>
                          )}
                        </td>
                      </tr>
                    ))}
    ...
    },
    {
      ...
      content: (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <StatCard label="Active Accounts" value={(customersData?.totalActiveCustomers || 0).toString()} icon={Users} loading={customersLoading} color="warning" />
            <StatCard label="Total Receivables" value={currency(financialData?.receivables?.total || 0)} icon={TrendingUp} loading={financialLoading} color="success" />
            <StatCard label="Total Payables" value={currency(financialData?.payables?.total || 0)} icon={TrendingDown} loading={financialLoading} color="danger" />
            <StatCard label="Net Liquidity" value={currency((financialData?.receivables?.total || 0) - (financialData?.payables?.total || 0))} icon={Wallet} loading={financialLoading} color="purple" />
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <Card className="border-none shadow-sm bg-white rounded-3xl overflow-hidden">
              <CardHeader className="p-8 pb-4">
                <CardTitle className="text-lg font-black text-slate-900 tracking-tight">Outstanding Receivables</CardTitle>
                <CardDescription className="text-xs">Top 5 debtors by ledger balance</CardDescription>
              </CardHeader>
              <CardContent className="p-0">
                <div className="divide-y divide-slate-50">
                  {customersData?.activeCustomers?.filter(c => (c.ledgerBalance || 0) > 0).slice(0, 5).map((customer, idx) => (
                    <div key={idx} className="px-8 py-5 flex items-center justify-between hover:bg-slate-50/30 transition-all">
                      <span className="font-bold text-slate-900">{customer.name || customer.company || "Unknown"}</span>
                      <span className="font-black text-slate-900 tabular-nums">{currency(customer.ledgerBalance || 0)}</span>
                    </div>
                  ))}
                </div>
              </CardContent>
            ...
            <Card className="border-none shadow-sm bg-white rounded-3xl overflow-hidden">
              <CardHeader className="p-8 pb-4">
                <CardTitle className="text-lg font-black text-slate-900 tracking-tight">Outstanding Payables</CardTitle>
                <CardDescription className="text-xs">Top 5 suppliers by outstanding credit</CardDescription>
              </CardHeader>
              <CardContent className="p-0">
                <div className="divide-y divide-slate-50">
                  {suppliersData?.topSuppliers?.slice(0, 5).map((supplier, idx) => (
                    <div key={idx} className="px-8 py-5 flex items-center justify-between hover:bg-slate-50/30 transition-all">
                      <span className="font-bold text-slate-900">{supplier.supplierName || supplier.company || "Unknown"}</span>
                      <span className="font-black text-rose-600 tabular-nums">{currency(supplier.totalAmount || 0)}</span>
                    </div>
                  ))}
                </div>
              </CardContent>
      ...
      <div className="pb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-black tracking-tighter text-slate-900 leading-none">Command Center</h1>
