    ...
    <head>
      ...
      <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { font-family: Arial, sans-serif; width: 100%; background: white; }
        .label { width: 100%; min-height: 25mm; padding: 1mm 2mm; display: flex; flex-direction: column; align-items: center; justify-content: center; overflow: hidden; }
        .composition { font-size: 7pt; font-weight: 600; color: #333; text-align: center; width: 100%; margin-bottom: 1mm; line-height: 1.2; }
        .price { font-size: 8pt; font-weight: 800; color: #000; text-align: center; width: 100%; margin-bottom: 1mm; }
        .barcode-img { display: flex; justify-content: center; align-items: center; width: 100%; padding: 1mm 0; }
        .barcode-img img { max-width: 100%; height: auto; max-height: 15mm; object-fit: contain; }
      </style>
    </head>
    <body>
      <div class="label">
...
export default function StockPage() {
  ...
  const [packetSearch, setPacketSearch] = useState("");
  const [packetAppliedSearch, setPacketAppliedSearch] = useState("");
  const [packetStockFilter, setPacketStockFilter] = useState("all");
  const [packetTypeFilter, setPacketTypeFilter] = useState("packet");
  const [copiedBarcode, setCopiedBarcode] = useState(null);
  const [packetToBreak, setPacketToBreak] = useState(null);
  const [printLoadingId, setPrintLoadingId] = useState(null);

  // CHANGED: Query configuration sets a large limit to grab everything for client-side evaluation
  const inventoryParams = useMemo(() => ({
    page: 1,
    limit: 5000,
  }), []);
  ...
  // CHANGED: Processes sliding sheet metrics locally before pipeline flows into individual column targets
  const filteredInventoryItems = useMemo(() => {
    let items = inventoryItems;

    // Sliding sheet logic processing blocks
    if (appliedFilters.lowStock) {
      items = items.filter(item => item.lowStock || item.needsReorder);
    }
    if (appliedFilters.needsReorder) {
      items = items.filter(item => item.needsReorder);
    }
    if (appliedFilters.minStock?.trim()) {
      const minQty = Number(appliedFilters.minStock.trim());
      if (Number.isFinite(minQty)) items = items.filter(item => (item.currentStock || 0) >= minQty);
    }
    if (appliedFilters.maxStock?.trim()) {
      const maxQty = Number(appliedFilters.maxStock.trim());
      if (Number.isFinite(maxQty)) items = items.filter(item => (item.currentStock || 0) <= maxQty);
    }
    if (appliedFilters.minCost?.trim()) {
      const minC = Number(appliedFilters.minCost.trim());
      if (Number.isFinite(minC)) items = items.filter(item => (item.averageCostPrice || 0) >= minC);
    }
    if (appliedFilters.maxCost?.trim()) {
      const maxC = Number(appliedFilters.maxCost.trim());
      if (Number.isFinite(maxC)) items = items.filter(item => (item.averageCostPrice || 0) <= maxC);
    }
    if (appliedFilters.minPrice?.trim()) {
      const minP = Number(appliedFilters.minPrice.trim());
      if (Number.isFinite(minP)) items = items.filter(item => (item.pricing?.minSellingPrice || item.pricing?.sellingPrice || 0) >= minP);
    }
    if (appliedFilters.maxPrice?.trim()) {
      const maxP = Number(appliedFilters.maxPrice.trim());
      if (Number.isFinite(maxP)) items = items.filter(item => (item.pricing?.minSellingPrice || item.pricing?.sellingPrice || 0) <= maxP);
    }
    if (appliedFilters.minValue?.trim()) {
      const minVal = Number(appliedFilters.minValue.trim());
      if (Number.isFinite(minVal)) items = items.filter(item => (item.totalValue || 0) >= minVal);
    }
    if (appliedFilters.maxValue?.trim()) {
      const maxVal = Number(appliedFilters.maxValue.trim());
      if (Number.isFinite(maxVal)) items = items.filter(item => (item.totalValue || 0) <= maxVal);
    }

    return items;
  }, [inventoryItems, appliedFilters]);

  // Ribbon Statistics calculations
  const totalStockValue = filteredInventoryItems.reduce((sum, item) => sum + (item.totalValue || 0), 0);
  const stockSummaryProducts = stockSummaryData?.products || [];
  const stockSummaryTotalValue = Number(stockSummaryData?.totalStockValue || 0);
  const stockSummaryTotalStockItems = stockSummaryProducts.reduce((sum, product) => sum + Number(product.stockInHand || 0), 0);
  const stockSummaryTotalProducts = Number(stockSummaryData?.totalItems || stockSummaryProducts.length || 0);
  const stockSummaryLowStockItems = Number(stockSummaryData?.lowStockItems || 0);
  const hasStockSummaryValue = stockSummaryData?.totalStockValue !== undefined;
  const hasStockSummaryItems = stockSummaryData?.products !== undefined;
  const hasStockSummaryLowStock = stockSummaryData?.lowStockItems !== undefined;
  ...
  const globalStockValue = (hasStockSummaryValue ? stockSummaryTotalValue : undefined) ?? inventoryValuationData?.summary?.totalInventoryValue ?? totalStockValue;
  const totalStockItems = filteredInventoryItems.reduce((sum, item) => sum + (item.currentStock || 0), 0);
  const lowStockCount = filteredInventoryItems.filter((item) => item.lowStock || item.needsReorder).length;
  const totalProducts = filteredInventoryItems.length;
  const globalTotalStockItems = (hasStockSummaryItems ? stockSummaryTotalStockItems : undefined) ?? totalStockItems;
  const globalLowStockCount = (hasStockSummaryLowStock ? stockSummaryLowStockItems : undefined) ?? lowStockCount;
  const globalTotalProducts = (hasStockSummaryProducts ? stockSummaryTotalProducts : undefined) ?? totalProducts;
  ...
  useEffect(() => { setMovementPage(1); }, [selectedProductId]);
  ...
  const { data: movementData } = useInventoryMovements(selectedProductId, movementParams, {
    enabled: Boolean(selectedProductId),
  });

  const { data: selectedInventory } = useInventoryItem(selectedProductId, {
    enabled: Boolean(selectedProductId),
  });

  // Packet Stock configurations
  const packetStockParams = useMemo(() => {
    const params = { page: packetPage, limit: packetPageLimit };
    if (packetAppliedSearch?.trim()) params.search = packetAppliedSearch.trim();
    if (packetStockFilter === "inStock") params.hasStock = "true";
    else if (packetStockFilter === "outOfStock") params.hasStock = "false";
    params.isLoose = "false";
    return params;
  }, [packetPage, packetPageLimit, packetAppliedSearch, packetStockFilter]);

  const { data: packetStockData, isLoading: packetStockLoading } = usePacketStockList(packetStockParams);
  const packetStockItems = packetStockData?.data ?? [];
  const packetStockPagination = packetStockData?.pagination;
  const adjustStockMutation = useAdjustStock();

  // Loose Stock definitions
  const [loosePage, setLoosePage] = useState(1);
  const [loosePageLimit, setLoosePageLimit] = useState(20);
  const [looseSearch, setLooseSearch] = useState("");
  const [looseAppliedSearch, setLooseAppliedSearch] = useState("");
  const [looseStockFilter, setLooseStockFilter] = useState("all");

  const looseStockParams = useMemo(() => {
    const params = { page: loosePage, limit: loosePageLimit, isLoose: "true" };
    if (looseAppliedSearch?.trim()) params.search = looseAppliedSearch.trim();
    if (looseStockFilter === "inStock") params.hasStock = "true";
    else if (looseStockFilter === "outOfStock") params.hasStock = "false";
    return params;
  }, [loosePage, loosePageLimit, looseAppliedSearch, looseStockFilter]);

  const { data: looseStockData, isLoading: looseStockLoading } = usePacketStockList(looseStockParams);
  const looseStockItems = looseStockData?.data ?? [];
  ...
  // CHANGED: Simplified column configurations using integrated filter types
  const inventoryColumns = useMemo(
    () => [
      {
        header: "Image",
      ...
      },
     {
      ...
      },
      {
        ...
        render: (row, { isExpanded } = {}) => {
          let sizes = [];
          const productSize = row.product?.size;
          if (Array.isArray(productSize) && productSize.length > 0) sizes = productSize;
          else if (productSize) sizes = [productSize];
          else if (row.raw?.variantComposition && Array.isArray(row.raw.variantComposition)) {
            const sizeSet = new Set();
      ...
      },
      {
        ...
        render: (row, { isExpanded } = {}) => {
          let colors = [];
          const productColor = row.product?.color;
          if (Array.isArray(productColor) && productColor.length > 0) colors = productColor;
          else if (productColor) colors = [productColor];
          else if (row.raw?.variantComposition && Array.isArray(row.raw.variantComposition)) {
            const colorSet = new Set();
            row.raw.variantComposition.forEach((v) => { if (v.color) colorSet.add(v.color); });
            colors = Array.from(colorSet);
          } else {
            const color = row.raw?.primaryColor || row.raw?.color || row.product?.specifications?.color;
            colors = Array.isArray(color) ? color : color ? [color] : [];
          }
          if (isExpanded) {
            return (
        ...
        pdfValue: (row) => row.product?.color || "—",
        filter: {
          value: filterForm.color,
          onChange: (val) => setFilterForm(prev => ({ ...prev, color: val })),
        }
      },
      {
        ...
        render: (row) => <div className="tabular-nums font-semibold">{formatNumber(row.currentStock)}</div>,
        pdfValue: (row) => row.currentStock || 0,
      },
      {
        header: "Landed Cost",
      ...
      },
      {
        ...
        className: "hidden md:table-cell",
        render: (row) => <span className="tabular-nums font-medium">{formatDecimal(row.pricing?.minSellingPrice ?? row.pricing?.sellingPrice ?? 0)}</span>,
        pdfValue: (row) => row.pricing?.minSellingPrice ?? row.pricing?.sellingPrice ?? 0,
      },
      {
        header: "Value",
      ...
      },
      {
        ...
        render: (row) => {
          const displayDate = row.firstArrivalDate || row.createdAt || row.lastStockUpdate;
          return <div className="text-sm text-muted-foreground">{displayDate ? new Date(displayDate).toLocaleDateString('en-GB') : "—"}</div>;
        },
        pdfValue: (row) => {
          const displayDate = row.firstArrivalDate || row.createdAt || row.lastStockUpdate;
          return displayDate ? new Date(displayDate).toLocaleDateString('en-GB') : "—";
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      if (result.success) toast.success("PDF report downloaded!");
  ...
  const handlePrintBarcode = async (packet) => {
    try {
      ...
      const response = await packetStockAPI.getBarcodeLabel(packet._id);
      const labelData = response.data?.data || response.data;
      const compositionText = (labelData?.composition || packet?.composition || []).map((c) => `${c.color}/${c.size} × ${c.quantity}`).join(", ") || "—";
      const unitMinPrice = Number(labelData?.minSellingPrice);
      const price = unitMinPrice > 0 ? unitMinPrice : 0;
  ...
  const inventoryTab = (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 rounded-[4px] border border-border bg-card/60 backdrop-blur-sm px-3 py-1.5 text-xs text-muted-foreground">
        <div className="flex items-center gap-1">
          <span>Stock Value:</span>
          <span className="font-semibold text-foreground">£{globalStockValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
        </div>
        <div className="h-3 w-px bg-border hidden sm:block" />
        <div className="flex items-center gap-1">
          <span>Total Items:</span>
        ...
        {activeFiltersCount > 0 && (
          <div className="flex flex-wrap items-center gap-1.5 py-1 text-xs border-t border-border/60 mt-1 pt-1.5">
            ...
            {appliedFilters.searchSupplier && (
              <Badge variant="secondary" className="h-5 text-[10px] gap-1 pl-2 pr-1 bg-muted border border-border">
                Supplier: {appliedFilters.searchSupplier}
                <button type="button" className="h-3 w-3 rounded-full hover:bg-muted-foreground/20 flex items-center justify-center font-bold" onClick={() => { setFilterForm(p => ({ ...p, searchSupplier: "" })); setAppliedFilters(p => ({ ...p, searchSupplier: "" })); }}>×</button>
              </Badge>
            )}
            {appliedFilters.searchProduct && (
              <Badge variant="secondary" className="h-5 text-[10px] gap-1 pl-2 pr-1 bg-muted border border-border">
                Product: {appliedFilters.searchProduct}
                <button type="button" className="h-3 w-3 rounded-full hover:bg-muted-foreground/20 flex items-center justify-center font-bold" onClick={() => { setFilterForm(p => ({ ...p, searchProduct: "" })); setAppliedFilters(p => ({ ...p, searchProduct: "" })); }}>×</button>
              </Badge>
            )}
            {appliedFilters.searchSku && (
              <Badge variant="secondary" className="h-5 text-[10px] gap-1 pl-2 pr-1 bg-muted border border-border">
                SKU: {appliedFilters.searchSku}
                <button type="button" className="h-3 w-3 rounded-full hover:bg-muted-foreground/20 flex items-center justify-center font-bold" onClick={() => { setFilterForm(p => ({ ...p, searchSku: "" })); setAppliedFilters(p => ({ ...p, searchSku: "" })); }}>×</button>
              </Badge>
            )}
            <Button variant="ghost" className="h-5 text-[10px] text-destructive hover:bg-destructive/5 px-2" onClick={handleResetFilters}>Clear All</Button>
          </div>
        )}
      </div>
...
const packetStockColumns = useMemo(() => [
    {
      header: "Barcode",
      accessor: "barcode",
      filterType: "text",
      filterValue: (row) => row.barcode || "—",
      pdfValue: (row) => row.barcode || "—",
      render: (row) => (
        <div className="flex items-center gap-2">
          <code className="px-2 py-1 bg-muted rounded text-xs font-mono">{row.barcode}</code>
          <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={(e) => { e.stopPropagation(); handleCopyBarcode(row.barcode); }}>
            {copiedBarcode === row.barcode ? <Check className="h-3 w-3 text-green-600" /> : <Copy className="h-3 w-3" />}
          </Button>
        </div>
      ),
    },
    {
      ...
      className: "hidden md:table-cell",
      filterType: "autocomplete",
      filterValue: (row) => row.isLoose ? "Loose" : "Packet",
      pdfValue: (row) => row.isLoose ? "Loose" : "Packet",
      render: (row) => <Badge variant={row.isLoose ? "secondary" : "default"}>{row.isLoose ? "Loose" : "Packet"}</Badge>,
    },
    {
      header: "Product",
      accessor: "product.name",
      filterType: "autocomplete",
      filterValue: (row) => row.product?.name || "—",
      pdfValue: (row) => row.product?.name || "—",
      render: (row) => (
        <div>
          <div className="font-medium">{row.product?.name || "—"}</div>
          <div className="text-xs text-muted-foreground">{row.product?.productCode || row.product?.sku || "—"}</div>
        </div>
      ),
    },
    {
      ...
      className: "hidden md:table-cell",
      filterType: "autocomplete",
      filterValue: (row) => row.supplier?.companyName || row.supplier?.company || row.supplier?.name || "—",
      pdfValue: (row) => row.supplier?.companyName || row.supplier?.company || row.supplier?.name || "—",
      render: (row) => {
        const companyName = row.supplier?.companyName || row.supplier?.company;
        const contactName = row.supplier?.name;
        if (!companyName && !contactName) return <div className="text-muted-foreground">—</div>;
        return (
          <div className="flex flex-col">
            <span className="font-medium text-foreground">{companyName || contactName}</span>
            {companyName && contactName && companyName !== contactName && <span className="text-[11px] text-muted-foreground">{contactName}</span>}
          </div>
        );
      },
    },
    {
      ...
      className: "hidden md:table-cell",
      filterType: "autocomplete",
      filterValue: (row) => (row.composition || []).map(c => `${c.color}/${c.size}`).join(", ") || "—",
      pdfValue: (row) => (row.composition || []).map(c => `${c.color}/${c.size}`).join(", ") || "—",
      render: (row) => {
        const comp = row.composition || [];
        if (comp.length === 0) return "—";
        return (
          <div className="flex flex-wrap gap-1 max-w-[200px]">
            {comp.slice(0, 3).map((c, idx) => (
              <span key={idx} className="inline-block px-1.5 py-0.5 bg-blue-50 text-blue-700 rounded text-[10px]">
                {c.color}/{c.size} ×{c.quantity}
              </span>
            ))}
            {comp.length > 3 && <span className="text-[10px] text-muted-foreground">+{comp.length - 3} more</span>}
          </div>
        );
      },
    },
    {
      ...
      className: "hidden md:table-cell",
      filterType: "text",
      filterValue: (row) => row.totalItemsPerPacket || 1,
      pdfValue: (row) => row.totalItemsPerPacket || 1,
      render: (row) => <div className="text-center tabular-nums">{row.totalItemsPerPacket || 1}</div>,
    },
    {
      header: "Available",
      accessor: "availablePackets",
      filterType: "text",
      filterValue: (row) => (row.availablePackets || 0) - (row.reservedPackets || 0),
      pdfValue: (row) => (row.availablePackets || 0) - (row.reservedPackets || 0),
      render: (row) => {
        const actual = (row.availablePackets || 0) - (row.reservedPackets || 0);
        return (
          <div className="text-right">
            <span className="font-medium tabular-nums">{actual}</span>
            {row.reservedPackets > 0 && <span className="text-xs text-amber-600 ml-1">({row.reservedPackets} reserved)</span>}
          </div>
        );
      },
    },
    {
      ...
      className: "hidden md:table-cell",
      filterType: "text",
      filterValue: (row) => row.product?.pricing?.minSellingPrice ?? row.product?.pricing?.sellingPrice ?? 0,
      pdfValue: (row) => row.product?.pricing?.minSellingPrice ?? row.product?.pricing?.sellingPrice ?? 0,
      render: (row) => <div className="text-right tabular-nums text-muted-foreground">{currency(row.product?.pricing?.minSellingPrice ?? row.product?.pricing?.sellingPrice)}</div>,
    },
    {
      header: "Actions",
      accessor: "_id",
      render: (row) => (
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="sm" className="h-7 w-7 p-0" disabled={printLoadingId === row._id} onClick={(e) => { e.stopPropagation(); handlePrintBarcode(row); }}>
            {printLoadingId === row._id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Printer className="h-4 w-4" />}
          </Button>
        </div>
      ),
    },
  ], [copiedBarcode, printLoadingId]); // Dependencies ensure actions re-render properly
  ...
  const packetStockSummary = useMemo(() => {
    let totalPackets = 0, totalItems = 0, totalValue = 0, lowStockCount = 0;
    (packetStockItems || []).forEach((p) => {
      const available = (p.availablePackets || 0) - (p.reservedPackets || 0);
      const perItemMin = Number(p.product?.pricing?.minSellingPrice ?? p.product?.pricing?.sellingPrice);
      const effectivePacketPrice = Number.isFinite(perItemMin) ? perItemMin * (p.totalItemsPerPacket || 1) : (p.suggestedSellingPrice || 0);
      totalPackets += available;
      totalItems += available * (p.totalItemsPerPacket || 1);
      totalValue += available * effectivePacketPrice;
      if (available > 0 && available <= 5) lowStockCount++;
  ...
  const looseStockSummary = useMemo(() => {
    let totalLoose = 0, totalValue = 0, lowStockCount = 0;
    (looseStockItems || []).forEach((p) => {
      const available = (p.availablePackets || 0) - (p.reservedPackets || 0);
      const perItemMin = Number(p.product?.pricing?.minSellingPrice ?? p.product?.pricing?.sellingPrice);
      totalLoose += available;
      totalValue += available * (Number.isFinite(perItemMin) ? perItemMin : (p.suggestedSellingPrice || 0));
      if (available > 0 && available <= 5) lowStockCount++;
    });
    return { totalLoose, totalValue, lowStockCount };
  }, [looseStockItems]);

  const looseStockTab = (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <Card><CardContent className="pt-4">Total Loose <div className="text-2xl font-bold">{formatNumber(looseStockSummary.totalLoose)}</div></CardContent></Card>
        <Card><CardContent className="pt-4">Total Value <div className="text-2xl font-bold">{currency(looseStockSummary.totalValue)}</div></CardContent></Card>
        <Card><CardContent className="pt-4">Low Stock <div className="text-2xl font-bold text-amber-600">{looseStockSummary.lowStockCount}</div></CardContent></Card>
      </div>
      <DataTableFiltered
        columns={packetStockColumns}
        data={looseStockItems}
  ...
  const packetStockTab = (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <Card><CardContent className="pt-4">Total Packets <div className="text-2xl font-bold">{formatNumber(packetStockSummary.totalPackets)}</div></CardContent></Card>
        <Card><CardContent className="pt-4">Total Value <div className="text-2xl font-bold">{currency(packetStockSummary.totalValue)}</div></CardContent></Card>
        <Card><CardContent className="pt-4">Low Stock <div className="text-2xl font-bold text-amber-600">{packetStockSummary.lowStockCount}</div></CardContent></Card>
      </div>
      {/* {JSON.stringify(packetStockItems)} */}
      <DataTableFiltered columns={packetStockColumns} data={packetStockItems} loading={packetStockLoading} paginate={false} compact={true} enableColumnFilters={true} />
    </div>
  );
  ...
  const addStockFields = [
    { name: "product", label: "Product", type: "combobox", required: true, placeholder: "Search product...", options: productOptions },
  ...
  const adjustStockFields = [
    {
      ...
      onChange: (value, currentData) => {
        const item = inventoryItems.find(i => i.productId === value);
        return item ? { ...currentData, newQuantity: item.currentStock || 0 } : currentData;
      }
    },
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <BackButton fallbackPath="/home" label="Back" />
          <h1 className="text-lg font-bold tracking-tight text-foreground ml-1.5 hidden sm:block">Stock & Inventory</h1>
        </div>
      </div>

      <Tabs tabs={tabs} activeTab={activeTab} onTabChange={handleTabChange} />

      <FormDialog open={addDialogOpen} title="Add Stock" fields={addStockFields} initialValues={{ product: selectedProductId, quantity: 1 }} onClose={() => setAddDialogOpen(false)} onSubmit={submitAddStock} />
      <FormDialog open={reduceDialogOpen} title="Reduce Stock" fields={reduceStockFields} initialValues={{ product: selectedProductId, quantity: 1 }} onClose={() => setReduceDialogOpen(false)} onSubmit={submitReduceStock} />
      <FormDialog open={adjustDialogOpen} title="Adjust Stock" fields={adjustStockFields} initialValues={{ product: selectedProductId, newQuantity: selectedInventory?.currentStock ?? 0 }} onClose={() => setAdjustDialogOpen(false)} onSubmit={submitAdjustStock} />

      <FormDialog
        open={minPriceDialogOpen}
        title="Update Minimum Selling Price"
        fields={[
          { name: "product", label: "Product", type: "combobox", required: true, placeholder: "Search product...", options: productOptions, onChange: (value, currentData) => { const item = inventoryItems.find(i => i.productId === value); return item ? { ...currentData, minSellingPrice: item.pricing?.minSellingPrice ?? item.pricing?.sellingPrice ?? 0, landedCost: item.averageCostPrice || 0 } : currentData; } },
          { name: "landedCost", label: "Landed Cost", type: "display", render: (data) => currency(data.landedCost || 0) },
          { name: "minSellingPrice", label: "Minimum Selling Price", type: "number", required: true, min: 0, step: 0.01 },
        ]}
        initialValues={{ product: selectedProductId, minSellingPrice: selectedInventory?.pricing?.minSellingPrice ?? selectedInventory?.pricing?.sellingPrice ?? 0, landedCost: selectedInventory?.averageCostPrice ?? 0 }}
        onClose={() => setMinPriceDialogOpen(false)}
        onSubmit={submitMinSellingPrice}
        loading={isSavingMinPrice}
      />
      ...
      <Sheet open={isFilterOpen} onOpenChange={setIsFilterOpen}>
        <SheetContent side="right" className="w-full sm:max-w-md flex flex-col p-0">
          <SheetHeader className="px-6 py-4 border-b">
            <SheetTitle className="text-base font-bold flex items-center gap-2"><Boxes className="h-5 w-5 text-primary" />Advanced Inventory Filters</SheetTitle>
          </SheetHeader>
          <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
            <div className="grid grid-cols-1 gap-3.5 text-xs">
              <div className="flex flex-col gap-1">
                <Label htmlFor="filter-min-stock">Min Stock</Label>
                <Input id="filter-min-stock" type="number" placeholder="Min qty" value={filterForm.minStock} onChange={(e) => setFilterForm(prev => ({ ...prev, minStock: e.target.value }))} />
              </div>
              <div className="flex flex-col gap-1">
                <Label htmlFor="filter-max-stock">Max Stock</Label>
                <Input id="filter-max-stock" type="number" placeholder="Max qty" value={filterForm.maxStock} onChange={(e) => setFilterForm(prev => ({ ...prev, maxStock: e.target.value }))} />
              </div>
              <div className="flex flex-col gap-1">
                <Label htmlFor="filter-min-cost">Min Cost (£)</Label>
                <Input id="filter-min-cost" type="number" step="0.01" placeholder="Min cost" value={filterForm.minCost} onChange={(e) => setFilterForm(prev => ({ ...prev, minCost: e.target.value }))} />
              </div>
              <div className="flex flex-col gap-1">
                <Label htmlFor="filter-max-cost">Max Cost (£)</Label>
                <Input id="filter-max-cost" type="number" step="0.01" placeholder="Max cost" value={filterForm.maxCost} onChange={(e) => setFilterForm(prev => ({ ...prev, maxCost: e.target.value }))} />
              </div>
              <div className="flex flex-col gap-1">
                <Label htmlFor="filter-min-price">Min Price (£)</Label>
                <Input id="filter-min-price" type="number" step="0.01" placeholder="Min price" value={filterForm.minPrice} onChange={(e) => setFilterForm(prev => ({ ...prev, minPrice: e.target.value }))} />
              </div>
              <div className="flex flex-col gap-1">
                <Label htmlFor="filter-max-price">Max Price (£)</Label>
                <Input id="filter-max-price" type="number" step="0.01" placeholder="Max price" value={filterForm.maxPrice} onChange={(e) => setFilterForm(prev => ({ ...prev, maxPrice: e.target.value }))} />
              </div>
              <div className="flex flex-col gap-1">
                <Label htmlFor="sheet-filter-low-stock" className="flex items-center gap-2 cursor-pointer"><Switch id="sheet-filter-low-stock" checked={filterForm.lowStock} onCheckedChange={(c) => setFilterForm(p => ({ ...p, lowStock: c }))} /> Low Stock</Label>
              </div>
              <div className="flex flex-col gap-1">
                <Label htmlFor="sheet-filter-reorder" className="flex items-center gap-2 cursor-pointer"><Switch id="sheet-filter-reorder" checked={filterForm.needsReorder} onCheckedChange={(c) => setFilterForm(p => ({ ...p, needsReorder: c }))} /> Reorder</Label>
              </div>
            </div>
          </div>
          <div className="p-4 border-t bg-muted/30 flex gap-2">
            <Button variant="outline" className="flex-1 text-xs" onClick={() => { handleResetFilters(); setIsFilterOpen(false); }}>Reset All</Button>
            <Button className="flex-1 text-xs" onClick={(e) => { handleApplyFilters(e); setIsFilterOpen(false); }}>Apply Filters</Button>
          </div>
        </SheetContent>
      </Sheet>

      <BarcodeUpdatePriceDialog open={barcodeUpdateDialogOpen} onOpenChange={setBarcodeUpdateDialogOpen} onSuccess={async () => { await queryClient.invalidateQueries({ queryKey: inventoryKeys.all }); }} />
      <BreakPacketDialog open={!!packetToBreak} onOpenChange={(open) => !open && setPacketToBreak(null)} packetStock={packetToBreak} mode="inventory" onSuccess={() => setPacketToBreak(null)} />
    </div>
  );
}