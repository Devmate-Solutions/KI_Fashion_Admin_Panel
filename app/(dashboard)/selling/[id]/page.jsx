          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            {Number(financials?.shippingCost || 0) > 0 && (
              <div>
                ...
                <p className="font-medium text-lg text-orange-600">+ {currency(financials?.shippingCost)}</p>
                {sale.shippingBoxes > 0 && sale.logisticsBoxRateSnapshot > 0 && (
                  <p className="text-[10px] text-muted-foreground">
                    {sale.shippingBoxes} {sale.shippingBoxes === 1 ? 'box' : 'boxes'} × {currency(sale.logisticsBoxRateSnapshot)}
                  </p>
                )}
              </div>
            )}
