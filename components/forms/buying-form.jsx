...
import { PlusIcon, TrashIcon, UserPlusIcon, X, Image as ImageIcon, Plus } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function BuyingForm({ initialSuppliers = [], onSave }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const paymentStatus = totals.supplierPaymentAfterDiscount <= 0 || totals.remaining <= 0
        ? 'paid'
        : totalPaid > 0
          ? 'partial'
          : 'pending'
          ...
          try {
            ...
            const productData = {
              ...
              size: Array.isArray(row.size) && row.size.length > 0
                ? row.size.join(', ')
                : (typeof row.size === 'string' ? row.size : undefined),
              specifications: {
                color: Array.isArray(row.primaryColor) && row.primaryColor.length > 0
                  ? row.primaryColor.join(', ')
                  : (typeof row.primaryColor === 'string' ? row.primaryColor : undefined)
              },
              pricing: {
            ...
            for (const imageFile of rowImages) {
              try {
                ...
                if (uploadedImageUrl) {
                  ...
                  console.warn(`Image uploaded but no URL returned for ${imageFile.name}`)
                }
              } catch (uploadErr) {
      ...
      const payload = {
        ...
        remainingBalance: totals.remaining, // Already accounts for discount
        paymentStatus,
    ...
    <div className="space-y-6">
            ...
            >
              ...
              <SelectContent>
                {logisticsCompanies.map((company) => (
                  <SelectItem key={company._id || company.id} value={String(company._id || company.id)}>
                    {company.name}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/50">
              <tr className="border-b">
                ...
                <th className="text-right p-3 font-medium min-w-[100px]">Total Quantity</th>
                <th className="text-center p-3 font-medium min-w-[120px]">Packet Config</th>
                ...
                <tr key={row.id} className="border-b hover:bg-muted/30 transition-colors">
                                ...
                                if (product.images && Array.isArray(product.images) && product.images.length > 0) {
                                  ...
                                  product.images.forEach((url, idx) => {
                                    if (url && typeof url === 'string' && url.trim() !== '') {
                                      existingPreviews[`existing-${idx}`] = url.trim()
                                    }
                                  })
                                  setImagePreviews(prev => ({
                                    ...prev,
                    ...
                    {(() => {
                      ...
                      const allImages = [
                        ...existingImageKeys
                          ...
                          .filter(img => img !== null),
                        ...(existingImageKeys.length === 0 ? getImageArray(row).map((url, idx) => ({
                          id: `fallback-${idx}`,
                          url,
                          isExisting: true,
                        })) : []),
                        ...newFiles.map((file) => {
                          ...
                          return {
                            id: fileId,
                            url: existingPreviews[fileId] || URL.createObjectURL(file),
                            isExisting: false,
                            file: file,
                          }
                        }),
                          ...
                          {allImages.length === 0 ? (
                            ...
                            <>
                              <div
                                ...
                                onClick={() => {
                                  setImageGalleryState({ rowId: row.id, images: allImages, selectedIndex: 0 })
                                }}
                              >
                                <img
                                  src={allImages[0].url}
                  ...
                  <td className="p-2">
                    <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => {
                            setRowInputValues(prev => ({
                              ...prev,
                              [row.id]: { ...prev[row.id], primaryColor: e.target.value }
                            }))
                          }}
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                              e.preventDefault()
                              const value = (rowInputValues[row.id]?.primaryColor || '').trim()
                              if (value) {
                                const currentArray = Array.isArray(row.primaryColor) ? row.primaryColor : []
                                if (!currentArray.includes(value)) {
                                  updateRow(row.id, 'primaryColor', [...currentArray, value])
                                  setRowInputValues(prev => ({
                                    ...prev,
                                    [row.id]: { ...prev[row.id], primaryColor: '' }
                                  }))
                                }
                              }
                              e.target.focus()
                      ...
                      {Array.isArray(row.primaryColor) && row.primaryColor.length > 0 && (
                          ...
                          {row.primaryColor.slice(0, 2).map((color, idx) => (
                            ...
                            >
                              ...
                              <button
                                type="button"
                                onClick={() => {
                                  const updated = row.primaryColor.filter((_, i) => i !== idx)
                      ...
                      <div className="flex items-center gap-1">
                        <Input
                          ...
                          onChange={(e) => {
                            setRowInputValues(prev => ({
                              ...prev,
                              [row.id]: { ...prev[row.id], size: e.target.value }
                            }))
                          }}
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') {
                              e.preventDefault()
                              const value = (rowInputValues[row.id]?.size || '').trim()
                              if (value) {
                                const currentArray = Array.isArray(row.size) ? row.size : []
                                if (!currentArray.includes(value)) {
                                  updateRow(row.id, 'size', [...currentArray, value])
                                  setRowInputValues(prev => ({
                                    ...prev,
                          ...
                          {row.size.slice(0, 2).map((size, idx) => (
                            ...
                            >
                              ...
                              <button
                                type="button"
                                onClick={() => {
                                  const updated = row.size.filter((_, i) => i !== idx)
      ...
      <section className="rounded-lg border border-border bg-card p-6 shadow-sm">
        ...
        <div className="mb-6 p-3 bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 rounded-lg border-2 border-slate-200 dark:border-slate-700">
          <h3 className="text-sm font-semibold mb-2 text-slate-700 dark:text-slate-300">Pricing Breakdown</h3>
          {rows.length > 0 && (
            <div className="space-y-1 mb-3">
              {rows.map((row) => (
                <div key={row.id} className="flex justify-between items-center text-xs py-1 px-2 bg-white dark:bg-slate-800 rounded border border-slate-200 dark:border-slate-700">
                  <span className="truncate flex-1 mr-2" title={row.productName || row.productCode}>
                    {row.productName || row.productCode || 'Product'}
                  </span>
                  <span className="text-muted-foreground mr-3">
                    Landed: £{(row.landedPrice || 0).toFixed(2)}
                  </span>
                  <span className="font-semibold text-blue-700 dark:text-blue-400">
                    Total: £{(row.landedTotal || 0).toFixed(2)}
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 pt-2 border-t border-slate-300 dark:border-slate-600">
            <div>
              <div className="text-xs text-muted-foreground">Supplier Payment Total</div>
              <div className="text-base font-semibold text-slate-900 dark:text-slate-100">
                {totals.supplierPaymentTotal.toFixed(2)}
              </div>
            </div>
            <div>
              <div className="text-xs text-muted-foreground">Landed Total (Inventory)</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center p-3 bg-muted/30 rounded-md">
              <span className="text-sm font-medium">Supplier Payment Total</span>
            ...
            <div className="flex justify-between items-center p-3 bg-blue-50 dark:bg-blue-950/30 rounded-md border-2 border-blue-200 dark:border-blue-900">
              <span className="text-sm font-medium">Amount Owed to Supplier</span>
            ...
            <div className="flex justify-between items-center p-3 bg-slate-50 dark:bg-slate-900 rounded-md">
              <span className="text-sm font-medium text-muted-foreground">Landed Total (Inventory)</span>
      ...
      <Dialog open={showAddSupplier} onOpenChange={setShowAddSupplier}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="new-supplier-name">Supplier Name <span className="text-red-500">*</span></Label>
              <Input
        ...
        setNewSupplierPhoneAreaCode("")
            ...
            <Button
              ...
              disabled={isCreatingSupplier || !newSupplierName.trim() || !newSupplierPhone.trim()}
            >
            ...
            onRemove={(imageId) => {
              ...
              if (imageId.startsWith('existing-') || imageId.startsWith('fallback-')) {
                ...
                if (updatedImages.length === 0) {
                  ...
                  setImageGalleryState({ ...imageGalleryState, images: updatedImages })
                }
              } else {
                const newFiles = productImages[rowId] || []
                const fileToRemove = newFiles.find(f => getFileId(f) === imageId)
                if (fileToRemove) {
              ...
              const updateGallery = (attempt = 0) => {
                setTimeout(() => {
                  ...
                  if (allPreviewsReady || attempt >= 10) {
                    ...
                    setImageGalleryState(prev => prev ? { ...prev, images: updatedImages } : null)
                  } else {
                    // Retry after 200ms
      ...
      {packetModalOpen && packetModalProduct && (
        <PacketConfigurationModal
          ...
          activeItemId={packetModalProduct?.id ? String(packetModalProduct.id) : undefined}
