...
export const ledgerAPI = {
  ...
  /**
     * Get ALL supplier payment receipts globally (Dashboard view)
     */
  getAllSupplierReceipts: async (params = {}) => {
    const result = await apiClient.get('/ledger/suppliers/payment-receipts', { params });
    return result;
  },
  /**
  ...
  /**
   * Get buyer payment receipts for a buyer. (Proxies to payments endpoints)
   * @param {string} buyerId - The buyer ID
   * @param {Object} params - { limit?, offset? }
   */
  getBuyerPaymentReceipts: async (buyerId, params = {}) => {
    const result = await apiClient.get(`/payments/customer/${buyerId}`, { params });
    return result;
  },

  /**
   * Get a specific buyer payment receipt by payment number.
   * @param {string} buyerId - The buyer ID (for consistency)
   * @param {string} paymentNumber - The payment number (PAY-...)
   */
  getBuyerPaymentReceipt: async (buyerId, paymentNumber) => {
    const result = await apiClient.get(`/payments/${paymentNumber}/receipt`);
    return result;
  },

  /**
   * Reverse a buyer payment (proxied to payments reverse endpoint)
   * @param {string} buyerId - The buyer ID
   * @param {string} paymentNumber - The payment number
   * @param {string} reason - Reason for reversal
   */
  reverseBuyerReceipt: async (buyerId, paymentNumber, reason) => {
    const result = await apiClient.post(`/payments/${paymentNumber}/reverse`, { reason });
    return result;
  },

  // =====================================================
  // Logistics Payment Receipts
  // =====================================================

  /**
   * Get payment receipts for a logistics company
   * @param {string} logisticsCompanyId - The logistics company ID
   * @param {Object} params - Query parameters (limit, offset, dateFrom, dateTo)
   */
  getLogisticsPaymentReceipts: async (logisticsCompanyId, params = {}) => {
    const result = await apiClient.get(`/logistics-payables/receipts/${logisticsCompanyId}`, { params });
    return result;
  },

  /**
   * Get detailed view of a single payment receipt
   * @param {string} receiptId - The receipt ID
   */
  getLogisticsPaymentReceiptDetails: async (receiptId) => {
    const result = await apiClient.get(`/logistics-payables/receipts/${receiptId}/details`);
    return result;
  },
};
