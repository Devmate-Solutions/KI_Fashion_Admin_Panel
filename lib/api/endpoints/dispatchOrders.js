...
export const dispatchOrdersAPI = {
  ...
  // Get edit-impact analysis for a confirmed order (super-admin only)
  editImpact: async (id) => {
    const result = await apiClient.get(`/dispatch-orders/${id}/edit-impact`);
    return result;
  },

  // Edit financial fields on a confirmed order (super-admin only)
  editConfirmed: async (id, payload) => {
    const result = await apiClient.patch(`/dispatch-orders/${id}/edit-confirmed`, payload);
    return result;
  },

  getPacketStocks: async (id) => {
    const result = await apiClient.get(`/dispatch-orders/${id}/packet-stocks`);
    return result;
  },
};
