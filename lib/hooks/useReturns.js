...
// Create product return mutation with cache invalidation
export function useCreateProductReturn() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (data) => {
      const response = await returnsAPI.createProductReturn(data);
      return response?.data?.data || response?.data || response;
    },
    onSuccess: () => {
      // Invalidate returns queries to trigger refetch
