...
export const reportsKeys = {
  ...
  stockSummary: (filters) => [...reportsKeys.all, 'stock-summary', filters],
  ...
  monthEndDetailed: (filters) => [...reportsKeys.all, 'month-end-detailed', filters],
  topStats: (filters) => [...reportsKeys.all, 'top-stats', filters],
};
...
export const useTopStatsReport = (params = {}) => {
  return useQuery({
    queryKey: reportsKeys.topStats(params),
    queryFn: () => reportsAPI.getDashboardTopStats(params),
...
export const useStockSummaryReport = (params = {}) => {
  return useQuery({
    queryKey: reportsKeys.stockSummary(params),
    queryFn: () => reportsAPI.getStockSummaryReport(params),
    select: (response) => response.data?.data || response.data || {},
  });
};
...
export const useMonthEndDetailedReport = (params = {}) => {
  return useQuery({
    queryKey: reportsKeys.monthEndDetailed(params),
    queryFn: () => reportsAPI.getMonthEndDetailedReport(params),
    select: (response) => response.data?.data || response.data || [],
    enabled: !!(params.startDate && params.endDate),
  });
};
