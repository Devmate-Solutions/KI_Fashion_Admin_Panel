...
export function useSubmitApproval() {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      if (data?.status === 'pending' || data?.status === 202) {
        toast.success(data?.message || 'Backdated order submitted for approval of super admin');
      } else {
        toast.success('Order submitted for approval of super admin successfully');
      }
    },
    onError: (error) => {
...
export function useConfirmDispatchOrder() {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['cash-tracking'] });
      queryClient.invalidateQueries({ queryKey: ['ledger'] });
      if (data?.status === 'pending' || data?.status === 202) {
        toast.success(data?.message || 'Backdated confirmation submitted for approval of super admin');
      } else {
...
export function useReturnDispatchItems() {
  ...
  return useMutation({
    mutationFn: async (payload) => {
      ...
      const response = await dispatchOrdersAPI.returnItems(id, data);
      return response?.data?.data || response?.data || response;
    },
    onSuccess: (data, variables) => {
      // Invalidate dispatch orders list
      ...
      // Invalidate specific dispatch order
      queryClient.invalidateQueries({ queryKey: dispatchOrdersKeys.detail(variables.id) });
      // Invalidate packet stocks for this order
      queryClient.invalidateQueries({ queryKey: [...dispatchOrdersKeys.detail(variables.id), 'packet-stocks'] });
...
// Fetch packet stocks for a dispatch order
export function useDispatchOrderPacketStocks(id) {
  return useQuery({
    queryKey: [...dispatchOrdersKeys.detail(id), 'packet-stocks'],
    queryFn: async () => {
      const response = await dispatchOrdersAPI.getPacketStocks(id);
...
export function useDeleteDispatchOrder() {
  ...
  return useMutation({
    ...
    onSuccess: (data, id) => {
      ...
      // Invalidate purchases list
      queryClient.invalidateQueries({ queryKey: ['purchases'] });
      // Invalidate ledger, inventory, cash-tracking, and reports
      queryClient.invalidateQueries({ queryKey: ['ledger'] });
      queryClient.invalidateQueries({ queryKey: inventoryKeys.lists() });
      queryClient.invalidateQueries({ queryKey: ['cash-tracking'] });
      queryClient.invalidateQueries({ queryKey: ['reports'] });
      
      toast.success(data?.message || 'Dispatch order deleted successfully');
    },
    onError: (error) => {
