...
export async function exportMonthEndToExcel(data, columns, totalsRow, summary, filename = "month_end_report") {
  try {
    ...
    // Determine the data range for formulas dynamically based on column accessors
    ...
    columns.forEach((col, idx) => {
      if (col.accessor) colMap[col.accessor] = getColLetter(idx)
    })

    // Header is row 15, Data starts at row 16
    const dataStartRow = 16
    const dataEndRow = dataStartRow + data.length - 1
    const dataRange = (accessor) => {
      const col = colMap[accessor] || "A"
      return `${col}${dataStartRow}:${col}${dataEndRow}`
    }

    // Row 5: Total Sell (C5)
    aoa.push(["", { v: "Total Sell", s: STYLE_SUMMARY_LABEL }, { f: `SUM(${dataRange("totalSell")})`, s: STYLE_SUMMARY_VALUE }])

    // Row 6: Sell in Loss (C6)
    aoa.push(["",
      { v: "Sell in Loss", s: STYLE_SUMMARY_LABEL },
      { f: `SUMIFS(${dataRange("totalSell")}, ${dataRange("remarks")}, "Loss")`, s: STYLE_SUMMARY_VALUE },
      { f: `C6/C5`, s: STYLE_SUMMARY_PERCENT }
    ])

    // Row 7: Sell in Profit (C7)
    aoa.push(["",
      { v: "Sell in Profit", s: STYLE_SUMMARY_LABEL },
      { f: `SUMIFS(${dataRange("totalSell")}, ${dataRange("remarks")}, "Profit")`, s: STYLE_SUMMARY_VALUE },
      { f: `C7/C5`, s: STYLE_SUMMARY_PERCENT }
    ])

    // Row 8: Profit / Net Profit Box (C8, E8, F8)
    aoa.push(["",
      { v: "Profit", s: STYLE_SUMMARY_LABEL },
      { f: `SUMIFS(${dataRange("pnl")}, ${dataRange("remarks")}, "Profit")`, s: STYLE_SUMMARY_VALUE },
      { v: "Net Profit", s: STYLE_SUMMARY_LABEL_ALT },
      { f: `SUM(${dataRange("pnl")})`, s: STYLE_SUMMARY_VALUE_ALT },
      { f: `E8/C5`, s: STYLE_SUMMARY_PERCENT_ALT }
    ])

    // Row 9: Loss (C9)
    aoa.push(["", { v: "Loss", s: STYLE_SUMMARY_LABEL }, { f: `SUMIFS(${dataRange("pnl")}, ${dataRange("remarks")}, "Loss")`, s: STYLE_SUMMARY_VALUE_RED }])

    // Row 10: Return (C10)
    aoa.push(["", { v: "Return", s: STYLE_SUMMARY_LABEL }, { v: summary.totalReturn || 0, s: STYLE_SUMMARY_VALUE }])

    // Row 11: C/Amount (C11)
    aoa.push(["", { v: "C/Amount", s: STYLE_SUMMARY_LABEL }, { f: `C5-C6-C10`, s: STYLE_SUMMARY_VALUE_BOLD }])

    // Row 12: Commission (C12)
    const rate = (summary.commissionRate || 2.5) / 100
    aoa.push(["", { v: "Commission", s: STYLE_SUMMARY_LABEL }, { f: `FLOOR(C11*${rate}, 5)`, s: STYLE_SUMMARY_VALUE }])
